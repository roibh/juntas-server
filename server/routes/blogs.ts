...
class Route {
    ...
    public static followedblogs(req: express.Request, res: express.Response) {
        if (!req.query.user_id) return res.sendStatus(400);
        dal.connect((error: any, db: any) => {
            cache.get('Users', req.query.user_id, (userData: any) => {
                if (!userData.Blogs)
                    userData.Blogs = [];

                db.collection('Blogs').find({ "$or": [{ "_id": { "$in": userData.Blogs } }, { "UserId": req.query.user_id }] }).toArray((err, result) => {
    ...
    public static newBlog(req: express.Request, res: express.Response) {
        if (!req.body) return res.sendStatus(400);
        dal.connect((error: any, db: any) => {

            let newBlogObject = req.body;
            db.collection('Blogs').save(newBlogObject).then((result) => {
                res.json(result);
            });
        });
    ...
    public static saveBlog(req: express.Request, res: express.Response) {
        if (!req.body) return res.sendStatus(400);
        dal.connect((error: any, db: any) => {
    ...
    public static getPosts(req: express.Request, res: express.Response) {
        if (!req.query.blog_id) return res.sendStatus(400);
        dal.connect((error: any, db: any) => {

            let newPostObject = req.body;
            db.collection('Posts').find({ blog_id: req.query.blog_id }).toArray().then((result) => {

                res.json(result);

                // async.eachSeries(result, (item, callback) => {
                //     render(item.body, (err, output) => {
                //         item.body = output;
                //         callback();
                //     });
                // }, () => {
    ...
    public static savePost(req: express.Request, res: express.Response) {
        if (!req.body) return res.sendStatus(400);
        dal.connect((error: any, db: any) => {

            let newPostObject = req.body;
            if (newPostObject._id) {
                newPostObject._id = ObjectID(newPostObject._id);
            }
            else {
                newPostObject.created_at = new Date();
            }

            db.collection('Posts').save(newPostObject).then((result) => {
                if (result.ops)
    ...
    public static getBlog(req: express.Request, res: express.Response) {
        if (!req.query.id) return res.sendStatus(400);
        dal.connect((error: any, db: any) => {


            db.collection('Blogs').find({ '_id': ObjectID(req.query.id) }).toArray((err, result) => {
                res.json(result[0]);
            });
        });
        // if (!req.query.url) return res.sendStatus(400);
...
export function appRoute(app: express.Router) {
    ...
    app.route("/blogs/posts")
        .post(Route.savePost)
        .get(Route.getPosts);


    app.route("/blogs/followedblogs")
        .get(Route.followedblogs);
