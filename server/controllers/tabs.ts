...
@MethodConfig('User')
export class Tabs {
    @Method(Verbs.Post, '/tabs/deletegroup')
    public async deletegroup(@Body() body) {
        if (!body) throw new MethodError(400);

        var finalObject = {};
        const data = await dal.deleteCollection("Tabs", body._id);
        return new MethodResult({
            "result": "ok"
        });
    };
    @Method(Verbs.Post, '/tabs/unsubscribefollowers')
    public async unsubscribefollowers(@Body() body) {
        if (!body) throw new MethodError(400);
        var finalObject = {};
        const data = await dal.getSingle("Tabs", body._id);

        if (data.UserId !== body.userId) {
            data.Followers.splice(data.Followers.indexOf(body.userId), 1);
            var query = "INSERT INTO Tabs";
            const tab = await dal.query(query, data);
            return new MethodResult(data);
        }

    }

    @Method(Verbs.Post, '/tabs/subscribefollowers')
    ...
    @Method(Verbs.Post, '/tabs/tab')
    public async fillfollowers(@Body() body) {

        if (!body) throw new MethodError(400);
        var finalObject: any = {};
        const data = await dal.getSingle("Tabs", body._id);
        const dataSet = await dal.getSet(data.Followers, "Users", );
        for (var i = 0; i < data.length; i++) {
            delete data[i].Password;
            delete data[i].Email;
            finalObject[data[i]._id] = data[i];
        }
        return new MethodResult(finalObject);
    ...
    @Method(Verbs.Get, '/tabs/metadata')
    public async metadata(@Query('url') url: string) {
        if (!url) throw new MethodError(400);

        const db = await dal.connect();

        var hash = verifier.url2filename(url);
        const result = await db.collection("Metadata").find({
            hash: hash
        }).toArray();
